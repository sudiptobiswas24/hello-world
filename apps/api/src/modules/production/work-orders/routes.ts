...
import * as svc from './work-orders.service'
...
export async function workOrderRoutes(app: FastifyInstance) {
  ...
  app.post('/:id/start', async (req, reply) => reply.send(await svc.start((req.params as any).id)))
  app.post('/:id/complete', async (req, reply) => {
    const { actualQty } = req.body as any
    reply.send(await svc.complete((req.params as any).id, actualQty))
  })
}
